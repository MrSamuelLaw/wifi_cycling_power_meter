...
class WifiClient(WebSocketClient):
    ...
    in order to get rpm data from wireless sensor
    over wifi
    ...
    def opened(self):
        ...
        sets self's connected status to true
        """
    ...
    def closed(self, code: int, reason=None):
        """
        override of the super's opended method
        sets self's connected status to false
        """
        self.connected = False
        print(
            f"""connection closed
    ...
    def received_message(self, m):
        ...
        sets self's rpm value if received before timeout
